...
export default function UserCharList() {
  ...
  const addCharToCamp = useMutation(api.character.assignCharToCampaign)
  ...
  const [inGameDays, setInGameDays] = useState<number>(0);
    ...
    return (
      <div className="flex flex-col space-y-12">
        ...
        <div className="">
            <span className="text-xs text-neutral-500 tracking-wider">Active Characters &mdash;</span>
            
            {char?.map(char => {
            return (
                <div className="" key={char.title}>
                    {char.playerId === user?.id && char.campaignId.length > 1 && (
        ...
        <div className="">
            <span className="text-xs text-neutral-500 tracking-wider">Inactive Characters &mdash;</span>
            
            {char?.map(char => {
            return (
                <div className="" key={char._id}>
                    {char.campaignId.length < 1 && (
                                        ...
                                        <Dialog>
                                            ...
                                            <DialogContent  className="sm:max-w-[425px]">
                                                <DialogHeader>
                                                    ...
                                                    <DialogDescription>
                                                        This will make {char.title} an active character in that campaign. 
                                                    </DialogDescription>
                                                    <div className="flex flex-col pt-6 ml-1 "> 
                                                        {camps?.map((camp) => {
                                                        const isMember = camp.members.some(member => member.playerId === user?.id);
                                                            ...
                                                            return (
                                                                <div key={camp._id}>
                                                                {isMember && ( 
                                                                    <button type='submit' key={camp._id} 
                                                                        className="font-medium tracking-wide my-2 flex justify-start border-l-2 border-neutral-500 pl-2"
                                                                        onClick={() => {
                                                                            router.push(`/dashboard/campaigns/${camp._id}`)
                                                                            addCharToCamp({
                                                                                id: char._id,
                                                                                campaignId: camp._id,
                                                                            })
