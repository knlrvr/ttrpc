...
export default function DmgDealtChart({
    params
}: {
    params: { url: string }
}) {
  ...
  const currentCampaign = params.url;
    ...
    const characterData = character?.map((character) => ({
       ...
       dmgDealt: character.dmgDealt as number, 
    }))
    ...
    return (
        <Card className="w-full flex flex-col items-center justify-center min-h-56">
            ...
            <p className="text-xs text-neutral-500 flex justify-center mt-2">Damage Dealt</p>
        </Card>
    )
}